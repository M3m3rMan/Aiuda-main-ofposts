...
app.post('/api/ask', async (req, res) => {
  ...
  try {
    ...
    if (typeof ragAnswer === 'string' && ragAnswer.startsWith("Traducción simulada:")) {
      needsWebSearch = true;
    }
    
    // Check if RAG answer contains fallback phrases
  ...
  /Traducción simulada:/i
    ...
    const needsWebSearch = fallbackPhrases.some(phrase => {
  const answerText = typeof ragAnswer === 'string' ? ragAnswer.toLowerCase() : '';
  
  if (phrase instanceof RegExp) {
    return phrase.test(answerText); // Test regex against the answer
  ...
  return false;
    ...
    return res.json({
      question,
      answer: language === 'es' ? translated : finalAnswer,
      language,
      source,
      threadId: threadId || 'auto-generated',
      timestamp: new Date().toISOString()
